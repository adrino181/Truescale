...
import PreviewPerson from "@/components/Blocks/Cards/previewPerson";
...
import IconFlip from "@/components/Blocks/IconFlip";
import IconMapping from "@/components/Blocks/IconMapping";
...
const Trending = ({ handleModal }) => {
  ...
  const onScrollEnd = React.useCallback(async (interactionCount: number) => {
    // console.log(interactionCount);
    setFilters(prev => ({ ...prev, pageCount: interactionCount }));
    let response = await dispatch(loadMoreAction({ filters: { ...filters, pageCount: interactionCount } }));
    if (!response || !response.payload || response.payload.length === 0) {
      setDisableScrollLoading(true);
    }
  }, [dispatch]);
  ...
  const handleListen = useCallback(
    (id) => {
      dispatch(updateListening({ id }));
      dispatch(updateListen({ id }));
    },
    [dispatch]
  );
  ...
  const getPopupCardActions = useCallback((id) => {
    return [
      {
        buttonSlug: "follow",
        action: () => followAuthorHandler(id),
        label: "Follow",
        active: false,
      },
    ];
  ...
  const getUserPostAction = useCallback(() => {
    return [
      {
        buttonSlug: "post",
        action: () => handleModal(),
      ...
      //   label: "Image",
      //   active: false,
      //   // icon: () => IconMapping.image()
      // },
    ];
  }, []);
  const PostCardCallback = useCallback((post) => {
    return (
      <PostCard
        post={{
          ...post,
          ...
          popperEle: <PreviewPerson author={post.author} theme={theme} followAuthor={followAuthorHandler} />
        }}
      />
    )
  }, [followAuthorHandler, getUserAction, theme, user])
  return (
    <>
      <Box
        sx={{
        ...
        <Box sx={{ display: "flex", justifyContent: "center", padding: 0, alignItems: 'center' }}>
          ...
          <div
            ...
            <IconFlip
              ...
              buttonSlug={getUserPostAction()}
              theme={theme}
            />
            {/* <Popper sx={{ zIndex: 1, }} disablePortal={true} open={open} anchorEl={anchorEl} > */}
            {/*   <PreviewPerson author={author} theme={theme} /> */}
            {/* </Popper> */}
          </div>
          <div>
      ...
      <ScrollView
        ...
        disableLoading={disableScrollLoading}
      >
        {
