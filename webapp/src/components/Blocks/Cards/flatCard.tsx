...
export default function FlatCard({ post, format }: PostCardProp) {
  const { date } = useConverter(post);
  const theme = useTheme();
  const { blockData, _id, tags, postData, action, author, user } = post || {};

  const cardUrl = postData?.rurl || `post/${_id ? _id : ""}`;
  const isPremium = user?.isPremium;
  const username = author.firstName
  ...
  return (
    <Card
      sx={{
        ...
        py: 2,
        px: 1,
        boxShadow: "1px 2px 7px 1px #8080802e",
      }}
    >
      <>
        {username ? (
          <CardHeader
            avatar={
              <Avatar
                sx={{ bgcolor: "transparent", width: 25, height: 25 }}
