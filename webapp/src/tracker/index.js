((window) => {
  ...
  const endpoint = "https://truescale.in/api/analytics/send";
  // const endpoint = 'https://ffb4-2405-201-3007-89f6-1cfe-132b-727d-ad53.ngrok-free.app/api/analytics/send';
  const screen = `${width}x${height}`;
  const eventRegex = /data-truescale-event-([\w-_]+)/;
  const eventNameAttribute = _data + "truescale-event";
  ...
  const trackingDisabled = () =>
    (localStorage && localStorage.getItem("truescale.disabled")) ||
    (dnt && doNotTrack()) ||
  ...
  const send = (payload) => {
    ...
    if (typeof cache !== "undefined") {
      headers["x-truescale-cache"] = cache;
    }
